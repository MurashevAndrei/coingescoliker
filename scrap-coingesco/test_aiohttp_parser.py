...
async def get_html_io(client, url, proxies_list):
    prox = random.choice(proxies_list)
    ...
    async with client.get(url, proxy=proxy, headers=headers) as response:
        ...
        if response.status != 200:
            coin_name = url.split('/')[-1]
            errors[coin_name] = f"{response.status}"
            print(response.status)
        return await response.text()

async def get_data_io(client, url, proxies_list):
    data = await get_html_io(client, url, proxies_list)
    coin_name = url.split('/')[-1]
    try:
        print(graber(data, coin_name))
    except:
        print('Somithing wrong')

    #print(data)
    #print(f"Get-data-io {url}")

async def gen_main(url, proxies_list):
    #loop = asyncio.get_running_loop()
    async with aiohttp.ClientSession() as client:
        await get_data_io(client, url, proxies_list)
        client.close()
...
if __name__ == '__main__':
    ...
    urls = run_parser()
    #urls = ['https://wtfismyip.com/text', 'https://www.coingecko.com/en/coins/bitcoin', 'https://wtfismyip.com/text', 'https://wtfismyip.com/text', 'https://wtfismyip.com/text'] # test urls

    t0 = time()
    print(t0)
    loop = asyncio.get_event_loop()
    a = int(round(len(urls) / 50, 0))
    j = 50
    loop = asyncio.get_event_loop()
    for i in range(a):
        if j > len(urls):
            future = [asyncio.ensure_future(gen_main(url, PROXIES_LIST)) for url in urls[j-50:]]
        future = [asyncio.ensure_future(gen_main(url, PROXIES_LIST)) for url in urls[j-50:j]]
