...
def load_Mongo_twscrname():
    db = client.test.twscrname
    data_name = 'twscrname'
    data_ = db.find_one({'_id': data_name})
    data_list = data_.get('coins')
    return data_list
...
def coins_get_tw_accounts(coins_ids_list, communities_dict):
    data_list = load_Mongo_twscrname()
...
def handler(coins, proxies_list):
    ...
    a = int(round(len(coins) / len(proxies_list), 0))
    if a == 0:
        a = 1
    j = len(proxies_list)
    for i in range(a):
        t_now = time()
        if j > len(coins):
            if j == len(proxies_list):
                asyncio.run(gen_main(coins, proxies_list))
            else:
                asyncio.run(gen_main(coins[j-len(proxies_list):], proxies_list))
        else:
            asyncio.run(gen_main(coins[j-len(proxies_list):j], proxies_list))
